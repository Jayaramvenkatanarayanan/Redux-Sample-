...
app.post('/adduser', (req, res) => {
  ...
  employee
    ...
    .then((token) => {
      res
        .header('x-auth', token)
        .send(JSON.stringify({result: employee, Message: 'user added', status: true}));
...
app.post('/login', (req, res) => {
  ...
  EmployeeTable
    .findByLogin(body.email, body.password)
    .then((employee) => {
      return employee
        .generateAuthToken()
        .then((token) => {
          res
            .header('x-auth', token)
            .send({result: employee, Message: 'Login successful,session started', status: true});
        });
    })
    .catch((e) => {
      res
...
app.delete('/logout', Authcheck, (req, res) => {
  ...
  req
    ...
    .then(() => {
      res
        .status(200)
        .send(JSON.stringify({Message: 'Session closed', status: true}));
    }),
  (e) => {
    res
      .status(400)
      .send(JSON.stringify({result: e, status: false}));
  };

});

// Emp depart models Add
...
app.get('/getall_Departmets', Authcheck, (req, res) => {
  ...
  DepartTable
    .find({create_by_id: req.user._id})
    .then((result) => {
...
app.delete('/delete/:depart_id', Authcheck, (req, res) => {
  ...
  DepartTable
    .findOne({depart_id: departID})
    .count()
    .then((count) => {
      console.log(count);
      DepartTable
        .remove({depart_id: departID, create_by_id: req.user._id})
        .then((result) => {
...
app.patch('/update_Department', Authcheck, (req, res) => {
  ...
  //res.send(email);
  DepartTable
    .find({depart_id: depart_id})
    .count()
    .then((count) => {

      if (count === 1) {
        DepartTable.update({
          depart_id: body.depart_id,
          create_by_id: req.user._id
        }, {$set: body}).then((result) => {
